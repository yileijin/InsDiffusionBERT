...
This is for semi-autoregressive diffusion generation
"""
...
class DiscreteDiffusionMatrixBase(DiscreteDiffusionBase):
    ...
    def get_qt_given_q0(self,
        ...
        if self.supports_efficient_inference():
            ...
            if instruction_prob is not None:
                prob_at_time_t = instruction_weight * instruction_prob + (1 - instruction_weight) * prob_at_time_t
            if return_logits:
    ...
    def sample_and_compute_posterior_q(self,
        ...
        """Samples from q(x_{t+1} | x_0), then computes q(x_t | x_{t+1}, x_0).
        Args:
          x_0:batch input ids or one_hot if make_one_hot == True 
          an array containing x_0 samples. These are expected to be integral
            unless make_one_hot is False (in which case probabilities can be
            provided).
          t: the timestep to compute (as an int or integer array with shape that
            matches x_0.
        ...
        prob_at_time_t = self.get_qt_given_q0(q0=x_0,
                        t=t,
        ...
        if self.supports_efficient_get():
            ...
            prob_at_time_t_plus_one = self.get_qt_given_q0(q0=x_0,
                            t=t + step_size,
                            instruction_prob=instruction_prob,
                            instruction_weight=instruction_weight, 
                            word_freq_logits=word_freq_logits)
...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    def __init__(self,
        ...
        self.instruction_lambda = instruction_lambda * torch.sin(torch.arange(schedule.num_steps + 1, device=self.device) / schedule.num_steps * math.pi)
    ...
    def instruction_noise_fn(self, q0, t, word_freq_logits,
                             instruction_prob):
        if instruction_prob is None:
            raise NotImplementedError
        p = self.state[t]
        if word_freq_logits is None:
            word_freq_logits = self.word_freq.repeat(q0.size(0), 1).gather(1, q0.argmax(-1))
        ...
        word_freq_probs = word_freq_logits.unsqueeze(-1) * self.word_freq_lambda[t]
        instruction_prob_t = instruction_prob * self.instruction_lambda[t]
        q0 = q0 + instruction_prob_t
        p = torch.clip(p + word_freq_probs, 0., .999)
    ...
    def get(self, t):
        """
        Get the probability of the word distribution on time t
        t: torch.tensor, size(1)
        """
        beta = self.schedule(t)
        prob_t = (1 - beta) * torch.eye(self.dim) + beta * self._get_mask()
    ...
    def get_qt_given_q0(self,
        ...
        For example, for t = 0, it returns q0 unchanged.
        Args:
          q0: One-hot or original inputs
          an array of floats specifying a distribution over p(x_0).
          t: t in q(x_t | x_0).
          return_logits: if True, return the output logits
          instruction_prob:conditioned probability computed by contrast encoder
          epsilon: a small number to normalize logits conversion with, if needed.
        Returns:
          q(x_t | x_0).
        """
        ...
        if instruction_prob is not None:
            prob_at_time_t = q0 if t == 0 else self.instruction_noise_fn(q0, t, word_freq_logits, instruction_prob)
        else:
            prob_at_time_t = q0 if t == 0 else self.noise_fn(q0, t, word_freq_logits)
        
        prob_at_time_t_condition = torch.einsum("ij,...j",
                        self.get_qt_matrix(t), prob_at_time_t)
        if return_logits:
            return torch.log(prob_at_time_t_condition + epsilon)
        else:
            return prob_at_time_t_condition
...
def p_forward(
    ...
    condition: The condition context used in condition_fn
    condition_fn: A fucntion computes the conditioned distribution probability
...
def compute_kl_reverse_process(x_start,
    ...
    if instruction_fn is None:
        raise ValueError("For conditional generation, condition_function is always required")
    
    if instruction is None:
        instruction_prob = instruction_fn(x_start, t.repeat(len(x_start)), attention_mask=target_mask)
    else:
        instruction_prob = instruction_fn(instruction, t.repeat(len(x_start)), attention_mask=target_mask)
...
def discrete_diffusion_predict_fn(
    ...
    def sampling_step(step, state):
        ...
        instruction_prob = instruction_fn(instruction, torch.tensor(t).repeat(len(instruction)))

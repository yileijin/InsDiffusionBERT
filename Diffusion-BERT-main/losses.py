...
def InfoNCE_Loss(contrust_encoder, input_ids, word_freq_mask=None, noise_fn=None, summary_ids=None, temperature = 0.7):
    if word_freq_mask is None or noise_fn is None:
        raise NotImplementedError
    device = contrust_encoder.device
    batch_size, seq_len = input_ids.size()
    loss = torch.tensor(0., device=device)
    x_cons = contrust_encoder(input_ids)
    with torch.no_grad():
        x_str1_mom = contrust_encoder(input_ids, attention_mask=word_freq_mask)
        x_str2_mom = noise_fn(contrust_encoder)
        if summary_ids != None:
            x_str3_mom = contrust_encoder(summary_ids)
    for i in range(batch_size):
        positive_sample = torch.tensor(0., device=device)
        
        positive_sample = positive_sample + torch.exp(torch.dot(x_cons[i] , x_str1_mom[i]) / temperature)
        positive_sample = positive_sample + torch.exp(torch.dot(x_cons[i] , x_str2_mom[i]) / temperature)
        
        negative_sample = torch.tensor(0., device=device) 
            
        negative_sample = negative_sample + torch.sum(
            torch.exp(torch.sum(x_cons[i][None, :] * x_str1_mom, dim=1) / temperature)) - \
            torch.exp(torch.dot(x_cons[i] , x_str1_mom[i]) / temperature)
           
        negative_sample = negative_sample + torch.sum(
            torch.exp(torch.sum(x_cons[i][None, :] * x_str2_mom, dim=1) / temperature)) - \
            torch.exp(torch.dot(x_cons[i] , x_str2_mom[i]) / temperature)
        if summary_ids != None:
            negative_sample = negative_sample + torch.sum(
            torch.exp(torch.sum(x_cons[i][None, :] * x_str3_mom, dim=1) / temperature)) - \
            torch.exp(torch.dot(x_cons[i] , x_str3_mom[i]) / temperature)
        loss += - torch.log(positive_sample /negative_sample).to(device)
    return loss
...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    ...
    def instructional_noise_fn(self, q0, t, word_freq_logits,
                             instruction_prob):
        ...
        p = torch.clip(p + word_freq_probs + instruction_prob_t, 0., .999)
    ...
    def get_qt_given_q0(self,
        ...
        if instruction_prob is not None:
            prob_at_time_t = q0 if t == 0 else self.instructional_noise_fn(q0, t, word_freq_logits, instruction_prob)
        else:
...
def compute_kl_reverse_process(x_start,
    ...
    if instruction is not None:
        instruction_prob = instruction_fn(instruction, t.repeat(len(x_start)))
...
def discrete_diffusion_predict_fn(
  ...
  """
    ...
    def sampling_step(step, state):
        ...
        instruction_prob = instruction_fn(instruction, t.repeat(len(instruction)))

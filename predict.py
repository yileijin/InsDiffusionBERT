...
import nltk
from dataloader import InstructionLoader
...
parser.add_argument("--ckpt_path_m", default='', type=str, required=False)
parser.add_argument("--ckpt_path_i", default='', type=str, required=False)
args = parser.parse_args()

step_size = args.step_size
device = 'cuda:0'
model_name = 'bert-base-uncased'
model_ckpt_path_m = args.ckpt_path_m
model_ckpt_path_i = args.ckpt_path_i
predict_x0 = True
...
if kind == 'word_freq':
    import Semi_AR_Diffu as diffusion
...
ckpt_m = torch.load(model_ckpt_path_m)
ckpt_i = torch.load(model_ckpt_path_i)
...
instruction_data = InstructionLoader(tokenizer, args.task_name).my_load(splits=['instruction'])[0]
instruction_loader = torch.utils.data.DataLoader(instruction_data, batch_size=shape[0], collate_fn=instruction_data.collate_fn, num_workers=4, pin_memory=True)
...
with open(f'./temp.txt', 'a+') as fdata:
    with open(f'./generation_results/{name}_step_curve.txt', 'a+') as fcurve:
        ...
        wfs = []
        with torch.no_grad():
            for i, batch in enumerate(tqdm(instruction_loader)):
                start = time.time()

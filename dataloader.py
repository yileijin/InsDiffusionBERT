...
class LoaderForContras:
    def __init__(self, tokenizer):
        self.tokenizer = tokenizer

    def _load(self, task_name, split):
        dataset = datasets.load_dataset(f'{task_name}', split=split)
        print(f'Example in {split} set:')
        print(dataset[0])
        dataset = dataset.map(self.selection, batched=True, remove_columns='text')
        print(dataset)
        dataset = dataset.map(partial(self.convert_to_features, tokenizer=self.tokenizer), batched=True, remove_columns='selected_inputs')
        return dataset
    ...
    @staticmethod
    def selection(example_batch):
        example_batch = [example for example in example_batch['text'] if len(example)>=25]
        selected_inputs = {'selected_inputs': example_batch}
        return selected_inputs

    @staticmethod
    def convert_to_features(example_batch, tokenizer):
        input_encodings = tokenizer.batch_encode_plus(example_batch['selected_inputs'], max_length=50, truncation=True, add_special_tokens=False)
        encodings = {
            'input_ids': input_encodings['input_ids'],
            'attention_mask': input_encodings['attention_mask'],
        }
...
class InstructionLoader:
    def __init__(self, tokenizer, task_name, return_source_length=False):
        self.tokenizer = tokenizer
        self.return_source_length = return_source_length
        self.data_dir = './instruction_data'
        self.task_name = task_name

    @staticmethod
    def _convert_to_features_original(example_batch, tokenizer):
        q1 = tokenizer.batch_encode_plus(example_batch['instrcution'], max_length=128, truncation=True, add_special_tokens=False)
        return {
            'instruction': q1['input_ids'],
        }
    ...
    @staticmethod
    def convert_to_features(example_batch, tokenizer):
        q1 = tokenizer(example_batch['instruction'], max_length=512, truncation=True, padding=True)
        encodings = {
            'instruction': q1['input_ids'],
        }
    ...
    def collate_fn(batch_input, tokenizer):
        input_ids = pad_sequence([torch.tensor(
            [tokenizer.cls_token_id] + d['instruction'] + [tokenizer.sep_token_id]
...
def convert_to_features(example_batch, tokenizer):
    q1 = tokenizer(example_batch['src'], max_length=512, truncation=True, padding=True)
    q2 = tokenizer(example_batch['trg'], max_length=512, truncation=True, padding=True)
    encodings = {
            'source': q1['input_ids'],
            'target': q2['input_ids'],
        }
...
class XSumLoader(ConditionalLoader):
    def __init__(self, tokenizer, return_source_length=False):
        super(XSumLoader, self).__init__(tokenizer, return_source_length)
        self.task_name = 'XSum'

    @staticmethod
    def add_prompt(example):
        example['src'] = ' Context: ' + example['src'] + ' Summary: '
        example['trg'] = example['trg']
        return example

...
parser.add_argument("--seq_len", default=400, type=int, required=False, help='Max seq length in generation')
...
temperature = 1.0
batch_size = 32
# With one source sentence, generate MBR_size's sentence
...
def denoise_fn(input_ids, corrupted_input_ids, timestep, attention_mask, target_mask):
    """
    Return the logits after currupted data put into model
    """
...
def collate_fn(batch_input):
    instruction = pad_sequence([torch.tensor(d['source']) for d in batch_input], batch_first=True)
